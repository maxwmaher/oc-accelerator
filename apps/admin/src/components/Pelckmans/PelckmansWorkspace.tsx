...
import { api, Document, OfferType } from './api'
...
function Wizard() {
  ...
  const [type, setType] = useState<OfferType>(
    (query.get('type') as OfferType) || 'Bundle'
  )
    ...
    <Container maxW="5xl" py="8">
      ...
      {step === 2 && (
        <Stack>
          <Alert status="info">
            <AlertIcon />
            Product discovery is loaded from
            OrderCloud after the draft is saved;
            raw IDs are never requested for
            products.
          </Alert>
          ...
          {type !== 'Bundle' && (
            <FormControl isRequired>
              ...
              <Select
                placeholder="Choose from discovered categories"
                value={selector}
                onChange={(e) =>
                  setSelector(e.target.value)
                }
              >
                <option value="PEL_CATEGORY_DEMO">
                  Pelckmans discovered selection
                </option>
              </Select>
            </FormControl>
          )}
        ...
        <Card>
          <CardBody>
            ...
            <Text mt="4" color="gray.600">
              Save the draft to resolve current
              Individual Customer prices. Saving
              does not publish or approve
              anything.
            </Text>
          </CardBody>
        </Card>
    ...
    <Container maxW="5xl" py="8">
      ...
      <HStack mt="5">
        ...
        {o.status === 'Published' && (
          <Alert status="info">
            <AlertIcon />
            Native verification is available
            only after the verification endpoint
            is configured.
          </Alert>
        )}
      </HStack>

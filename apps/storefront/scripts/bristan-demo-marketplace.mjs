...
function bulkPriceSchedule(id, name, base, supplier) {
  return { ID: id, Name: name, ApplyTax: false, ApplyShipping: false, MinQuantity: 10, MaxQuantity: 10000, RestrictedQuantity: false, PriceBreaks: [[10, 0.9 - supplier.discount], [50, 0.82 - supplier.discount], [100, 0.76 - supplier.discount]].map(([Quantity, factor]) => ({ Quantity, Price: money(base * factor) })), xp: { Demo: 'BristanMarketplace', Journey: 'SupplierBuyingFromBristan' } };
}
...
async function seed() {
  ...
  for (const supplier of SUPPLIERS) {
    ...
    for (const p of products.slice(0, 12)) {
      const ps = bulkPriceSchedule(`bristan-demo-bulk-${supplier.key}-${p.id}`, `${supplier.name} bulk price - ${p.name}`, productPrice(p), supplier);
